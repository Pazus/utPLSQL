create or replace package body ut_annotations as
  ...
  */
  ...
  c_annot_comment_pattern     constant varchar2(30) := '^( |' || chr(09) || ')*-- *(' || gc_annotation_qualifier ||
                                                       '.*?)$'; -- chr(09) is a tab character
  ...
  c_annotation_block_pattern    constant varchar2(200) := '(({COMMENT#.+}' || chr(10) || ')+)( |' || chr(09) ||
                                                          ')*(procedure|function)\s+(' || c_rgexp_identifier || ')';
  ...
  begin
    ...
    --                                ,replacestr => chr(10)||chr(10));
        return  l_tmp_clob;
    */
    return regexp_replace(srcstr => a_source, pattern => c_multiline_comment_pattern, modifier => 'n');
  ...
  begin
    ...
    return case when l_package_comments is not null then get_annotations(l_package_comments, a_comments) end;
  end;
      ...
      while l_name is not null loop
        ...
        if a_annotated_pkg.procedure_annotations(i).annotations(l_name).count > 0 then
          ...
          for j in 1 .. a_annotated_pkg.procedure_annotations(i).annotations(l_name).count loop
            dbms_output.put_line('      ' || nvl(a_annotated_pkg.procedure_annotations(i).annotations(l_name)(j).key
                                                ,'<Anonymous>') || ' = ' ||
