create or replace type body ut_custom_reporter is
  ...
  overriding member procedure begin_assert(self in out nocopy ut_custom_reporter, an_assert_message in varchar2) as
  begin
    (self as ut_dbms_output_suite_reporter).begin_assert(an_assert_message);
    lvl := lvl + 1;
  end;

  overriding member procedure end_assert(self in out nocopy ut_custom_reporter, an_assert ut_assert_result)as
  begin
    lvl := lvl - 1;
    (self as ut_dbms_output_suite_reporter).end_assert(an_assert);
  end;

  overriding member procedure end_test(self in out nocopy ut_custom_reporter, a_test_name in varchar2, a_test_call_params in ut_test_call_params, a_execution_result in ut_execution_result) as
  begin
    lvl := lvl - 1;
    (self as ut_dbms_output_suite_reporter).end_test(a_test_name, a_test_call_params, a_execution_result);
  end;
